...
namespace ungdungthuviencaocap
{
	class Modify
	{
		...
		internal bool CheckDuplicateBook(sachquanly sach)
		{
			...
			if (sach.ID > 0) // Nếu là cập nhật, loại trừ sách hiện tại khỏi kiểm tra trùng lặp
			{
				query += " AND ID != @ID";
			}
			...
			using (SQLiteConnection sqlConnection = Connection.GetSQLiteConnection())
			{
				try
				{
					...
					using (SQLiteCommand sqlCommand = new SQLiteCommand(query, sqlConnection))
					{
						...
						if (sach.ID > 0)
						{
							sqlCommand.Parameters.AddWithValue("@ID", sach.ID);
						}
		...
		{
			// Kiểm tra trùng lặp trước khi cập nhật, loại trừ chính sách đang sửa
			if (CheckDuplicateBook(sachquanly)) // CheckDuplicateBook cần được sửa để xử lý trường hợp cập nhật
			{
				MessageBox.Show("Thông tin sách (Tên, Thể loại, Tác giả, NXB, Năm XB) bị trùng với sách khác.", "Lỗi Trùng Lặp", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return false;
			}
		...
		{
			DataTable dataTable = new DataTable();
			// Lấy tất cả các cột bao gồm cả DoiTuongMuon
			string query = "SELECT ID, TenSach, MaSinhVien, HoVaTen, LoaiPhieu, NgayMuon, NgayTra, SoLuong, MaSach, DoiTuongMuon, IsActive FROM muontrasach ORDER BY TenSach ASC";
			using (SQLiteConnection sqlConnection = Connection.GetSQLiteConnection())
			{
				sqlConnection.Open();
				dataAdapter = new SQLiteDataAdapter(query, sqlConnection);
		...
		// Phương thức thêm phiếu mượn, bao gồm cả DoiTuongMuon
		public bool themphieu(muontra muontra)
		{
			// Câu lệnh SQL INSERT bao gồm cả cột DoiTuongMuon
			string query = @"INSERT INTO muontrasach (TenSach, MaSinhVien, HoVaTen, LoaiPhieu, NgayMuon, NgayTra, SoLuong, MaSach, DoiTuongMuon, IsActive) 
                    VALUES (@TenSach, @MaSinhVien, @HoVaTen, @LoaiPhieu, @NgayMuon, @NgayTra, @SoLuong, @MaSach, @DoiTuongMuon, @IsActive)";
			...
			using (SQLiteConnection sqlConnection = Connection.GetSQLiteConnection())
			{
				try
				{
					...
					using (SQLiteCommand sqlCommand = new SQLiteCommand(query, sqlConnection))
					{
						...
						sqlCommand.Parameters.Add("@MaSinhVien", DbType.String).Value = muontra.Masinhvien; // Đảm bảo kiểu dữ liệu nhất quán
						...
						sqlCommand.Parameters.Add("@SoLuong", DbType.Int32).Value = Convert.ToInt32(muontra.Soluong); // Chuyển đổi sang Int32
						sqlCommand.Parameters.Add("@MaSach", DbType.String).Value = muontra.Masach;
						sqlCommand.Parameters.Add("@DoiTuongMuon", DbType.String).Value = muontra.DoiTuongMuon; // Thêm tham số DoiTuongMuon
						sqlCommand.Parameters.Add("@IsActive", DbType.Int32).Value = muontra.IsActive;
		...
		// Phương thức sửa phiếu mượn, bao gồm cả DoiTuongMuon
		public bool suaphieu(muontra muontra)
		{
			// Câu lệnh SQL UPDATE bao gồm cả cột DoiTuongMuon
			string query = @"UPDATE muontrasach SET TenSach = @TenSach, MaSinhVien = @MaSinhVien, HoVaTen = @HoVaTen, 
                    LoaiPhieu = @LoaiPhieu, NgayMuon = @NgayMuon, NgayTra = @NgayTra, SoLuong = @SoLuong, MaSach = @MaSach, DoiTuongMuon = @DoiTuongMuon, IsActive = @IsActive
                    WHERE ID = @ID";
			...
			using (SQLiteConnection sqlConnection = Connection.GetSQLiteConnection())
			{
				try
				{
					...
					using (SQLiteCommand sqlCommand = new SQLiteCommand(query, sqlConnection))
					{
						...
						sqlCommand.Parameters.Add("@NgayMuon", DbType.Date).Value = muontra.Ngaymuon;
						sqlCommand.Parameters.Add("@NgayTra", DbType.Date).Value = muontra.Ngaytra;
						sqlCommand.Parameters.Add("@SoLuong", DbType.Int32).Value = Convert.ToInt32(muontra.Soluong);
						sqlCommand.Parameters.Add("@MaSach", DbType.String).Value = muontra.Masach;
						sqlCommand.Parameters.Add("@DoiTuongMuon", DbType.String).Value = muontra.DoiTuongMuon; // Thêm tham số DoiTuongMuon
						sqlCommand.Parameters.Add("@IsActive", DbType.Int32).Value = muontra.IsActive;
		...
		public bool trasach(int ID)
		{
			...
			{
				sqlConnection.Open();
				// Nên sử dụng using cho SQLiteCommand để đảm bảo giải phóng tài nguyên
				using (SQLiteCommand sqlcommand = new SQLiteCommand(query, sqlConnection))
				{
					sqlcommand.Parameters.AddWithValue("@ID", ID);
				...
				Console.WriteLine($"DEBUG Modify.trasach: Attempted to update muontrasach for ID='{ID}'. Rows affected: {rowsAffected}");
				return rowsAffected > 0; // Trả về true nếu có ít nhất 1 dòng được cập nhật
			}
			catch (Exception ex)
			{
				Console.WriteLine($"ERROR Modify.trasach for ID='{ID}': {ex.ToString()}");
				MessageBox.Show($"Lỗi khi cập nhật trạng thái trả sách (ID: {ID}): {ex.Message}", "Lỗi Hệ Thống", MessageBoxButtons.OK, MessageBoxIcon.Error);
			...
			// Dòng này sẽ không bao giờ đạt được nếu cấu trúc try-catch-finally có return bên trong.
			// Tuy nhiên, để đảm bảo, nếu logic thay đổi, nó vẫn trả về kết quả dựa trên rowsAffected.
			// return rowsAffected > 0; 
		}
		...
		{
			DataTable dt = new DataTable();
			// Lấy tất cả các cột bao gồm DoiTuongMuon và IsActive
			string query = "select ID, TenSach, MaSinhVien, HoVaTen, LoaiPhieu, NgayMuon, NgayTra, SoLuong, MaSach, DoiTuongMuon, IsActive from muontrasach where MaSinhVien LIKE @MaSinhVien";
		...
		// Phương thức lấy mã sách dựa trên tên sách
		public string GetMaSachByTenSach(string tenSach)
		{
			string maSach = null;
			string query = "SELECT MaSach FROM quanlysach WHERE TenSach = @TenSach COLLATE NOCASE LIMIT 1";
			using (SQLiteConnection conn = Connection.GetSQLiteConnection())
			{
				try
				{
				...
				{
					Console.WriteLine($"Lỗi khi lấy mã sách theo tên '{tenSach}': {ex.Message}");
					MessageBox.Show($"Lỗi khi lấy mã sách: {ex.Message}", "Lỗi CSDL", MessageBoxButtons.OK, MessageBoxIcon.Error);
				}
			}
			return maSach;
		}

		// Phương thức kiểm tra số lượng sách đang mượn của một người dùng
		public int GetSoLuongSachDangMuon(string maNguoiMuon, string doiTuongMuon)
		{
			int soLuongDangMuon = 0;
			// Chỉ đếm các phiếu đang active (IsActive = 1)
			string query = "SELECT SUM(SoLuong) FROM muontrasach WHERE MaSinhVien = @MaNguoiMuon AND DoiTuongMuon = @DoiTuongMuon AND IsActive = 1";
			using (SQLiteConnection conn = Connection.GetSQLiteConnection())
			{
				try
				{
					...
					using (SQLiteCommand cmd = new SQLiteCommand(query, conn))
					{
						...
						cmd.Parameters.AddWithValue("@DoiTuongMuon", doiTuongMuon);
						object result = cmd.ExecuteScalar();
						if (result != null && result != DBNull.Value)
						{
							soLuongDangMuon = Convert.ToInt32(result);
						}
					}
				}
				catch (Exception ex)
				{
					Console.WriteLine($"Lỗi khi kiểm tra số lượng sách đang mượn của '{maNguoiMuon}': {ex.Message}");
					MessageBox.Show($"Lỗi khi kiểm tra số lượng sách đang mượn: {ex.Message}", "Lỗi CSDL", MessageBoxButtons.OK, MessageBoxIcon.Error);
					// Trả về một giá trị lớn để ngăn việc mượn thêm nếu có lỗi
					return int.MaxValue;
				}
			}
